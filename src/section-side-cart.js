if (!customElements.get("s-side-cart")) {
  customElements.define(
    "s-side-cart",
    class SectionSideCart extends HTMLElement {
      constructor() {
        super();
      ...
      connectedCallback() {
        ...
        this.headingStart = this.querySelector(".js-side-cart__heading-start");
        ...
        this.closeSideCartUnsubscriber = window.PubSub.subscribe("close-side-cart", this.closeSideCart);
      ...
      disconnectedCallback() {
        ...
        this.closeSideCartUnsubscriber();
      ...
      updateVariables = () => {
        ...
        this.itemsContainer = this.querySelector(".js-side-cart__cart-content");
        this.deliveryBannerText = this.querySelector(".js-side-cart__free-delivery-text");
        this.cartItemCount = this.querySelector(".js-side-cart__counter");
        this.deliveryWrapper = this.querySelector(".js-side-cart__delivery-wrapper");
        this.overlay = this.querySelector(".js-side-cart__overlay");
        this.closeButton = this.querySelector(".js-side-cart__close");

        this.discountForm = this.querySelector(".js-side-cart__discount-wrap");

        this.sliderRenderContainerDesktop = this.querySelector(".js-side-cart__slider-render-container--desktop");
        this.sliderRenderContainerMobile = this.querySelector(".js-side-cart__slider-render-container--mobile");
        this.sliderWrapperDesktop = this.querySelector(".js-side-cart__recommendations-slider--desktop");
        this.sliderWrapperMobile = this.querySelector(".js-side-cart__recommendations-slider--mobile");

        this.qtyInputs = Array.from(this.querySelectorAll(".js-side-cart__form-qty-input"));
        this.qtyButtonsPlus = Array.from(this.querySelectorAll(".js-side-cart__form-qty-button--plus"));
        this.qtyButtonsMinus = Array.from(this.querySelectorAll(".js-side-cart__form-qty-button--minus"));
      ...
      openSideCart = () => {
        this.updateContainerTopOffset();
        this.classList.add("is-active");
        document.body.classList.add("side-cart-is-active");
      };

      updateContainerTopOffset = () => {
        const overlay = this.querySelector(".js-side-cart__overlay");
        const container = this.querySelector(".js-side-cart__container");
        ...
        overlay.style.top = `${announcementBarHeight}px`;
        container.style.top = `${announcementBarHeight}px`;
      };

      closeSideCart = () => {
        this.classList.remove("is-active");
        document.body.classList.remove("side-cart-is-active");
      };
      ...
      handleClick = async (event) => {
        ...
        if (target.closest(".js-side-cart__discount-submit-btn")) {
          this.handleDiscountSubmission(event);
        } else if (target.closest(".js-side-cart__discount-toggle")) {
          this.classList.add("is-discount-form-active");
        } else if (target.closest(".js-side-cart__discount-remove")) {
          this.removeDiscount(target);
        } else if (target.closest(".js-side-cart__remove-all")) {
          this.clearCart();
        }

        if (target.closest(".js-side-cart__cart-remove")) {
          const item = target.closest(".js-side-cart__item");
          const itemToDelete = {
        ...
        if (target.closest(".js-side-cart__wishlist-add, .js-side-cart__wishlist-remove")) {
          const button = target.closest(".s-side-cart__wishlist-btn");
          const item = target.closest(".js-side-cart__item");
        ...
        if (target.closest(".js-side-cart__toggle")) {
          this.closeAllDropDown();
          const itemInner = target.closest(".js-side-cart__item-inner");

          const toggle = target.closest(".js-side-cart__toggle");
        ...
        if (target.closest(".js-side-cart__option")) {
          const oldProduct = target.closest(".js-side-cart__item");
          const itemToDelete = {
      ...
      handleDiscountSubmission = (event) => {
        ...
        const discountCodesList = this.querySelector(".js-side-cart__discounts-list")?.dataset.codes;
        const discountCodes = discountCodesList?.split(",") || [];
        const discountCode = this.querySelector(".js-side-cart__discount-input").value.trim();
        this.hideError();
        ...
        this.updateDiscountCodes(discountCodes)
          .then((cart) => {
            if (!cart.discount_codes.at(-1)?.applicable) {
              this.querySelector(".js-side-cart__discount-container").classList.add("is-not-valid");
            } else {
      ...
      removeDiscount = (target) => {
        const discountCodesList = this.querySelector(".js-side-cart__discounts-list")?.dataset.codes;
        const discountcodes = discountCodesList.split(",") || [];
        const discountCode = target.closest(".js-side-cart__discount-remove").dataset.code;
        this.hideError();
      ...
      qtyInputChangeHandler = (event) => {
        ...
        const qtyContainer = input.closest(".js-side-cart__item-quantity");
        const buttonPlus = qtyContainer.querySelector(".js-side-cart__form-qty-button--plus");
        const buttonMinus = qtyContainer.querySelector(".js-side-cart__form-qty-button--minus");
        ...
        buttonPlus.classList.toggle("s-side-cart__form-qty-button--disabled", clampedValue >= max);
        buttonMinus.classList.toggle("s-side-cart__form-qty-button--disabled", clampedValue <= min);
      ...
      updateQuantity = async (input) => {
        const cartItem = input.closest(".js-side-cart__item");
      ...
      clearCart = async () => {
        try {
          ...
          const cart = await response.json();
          const allItems = Array.from(this.querySelectorAll(".js-side-cart__item"));
      ...
      closeAllDropDown = () => {
        Array.from(this.querySelectorAll(".js-side-cart__item-inner")).forEach((inner) => inner.classList.remove("is-visible-options"));
        Array.from(this.querySelectorAll(".js-side-cart__toggle")).forEach((dropdown) => dropdown.classList.remove("is-active"));
      };

      closeVariantsDropDown = () => {
        Array.from(this.querySelectorAll(".js-side-cart__item-inner.is-visible-options")).forEach((inner) => inner.classList.remove("is-visible-options"));
        Array.from(this.querySelectorAll(".js-side-cart__toggle.is-active")).forEach((toggle) => toggle.classList.remove("is-active"));
      };
      ...
      updateCartContent = async () => {
        ...
        await fetch(url)
          ...
          .then((response) => {
            ...
            const newWrapperTemplate = parsedHTML.querySelector(".t-side-cart__body");
            const newFooterTemplate = parsedHTML.querySelector(".t-side-cart__footer");
            ...
            const oldWrapper = this.querySelector(".js-side-cart__body");
            const oldFooter = this.querySelector(".js-side-cart__footer");
            ...
            const cartItems = Array.from(newWrapper.querySelectorAll(".js-side-cart__item"));
      ...
      updateSingleItem = async (key) => {
        const res = await fetch(`/cart?section_id=side-cart-items`);
      ...
      updateTotals = (cart) => {
        ...
        this.querySelector(".js-side-cart__summary-container").innerHTML = summaryHTML;
      ...
      updateDelivery = (cart) => {
        ...
        const bar = this.deliveryWrapper.querySelector(".s-side-cart__free-delivery-bgd");
      ...
      renderEmptyState = () => {
        const thisCartContentWrapper = this.querySelector(".js-side-cart__cart-content-wrapper");
        const checkoutButton = this.querySelector(".js-side-cart__button-checkout");
        const discountContainer = this.querySelector(".js-side-cart__discount-container");
        const removeAllButton = this.querySelector(".js-side-cart__remove-all");
        ...
        this.updateDelivery({ total_price: 0 });
        this.initYMAL();
      };
      ...
      checkInWishlistAllProductsSwym = () => {
        const cartItems = Array.from(this.querySelectorAll(".js-side-cart__item"));
      ...
      initYMAL = async (cart = null) => {
        const isStatic = this.sliderRenderContainerDesktop?.classList.contains("is-static") && 
                         this.sliderRenderContainerMobile?.classList.contains("is-static");
        if (isStatic) {
          return;
        }
        ...
        try {
          ...
          const res = await fetch(`${window.Shopify.routes.root}recommendations/products?product_id=${productId}&limit=6&intent=related&section_id=side-cart-ymal`);
          ...
          const parser = new DOMParser();
          const doc = parser.parseFromString(html, "text/html");
          const slideContainer = doc.querySelector(".js-side-cart__slider-container");
          ...
          const slideCounts = Array.from(doc.querySelectorAll(".js-side-cart__slide")).length;
          ...
          const desktopSlider = slideContainer.querySelector(".s-side-cart__slider--desktop");
          if (this.sliderRenderContainerDesktop && desktopSlider) {
            this.sliderRenderContainerDesktop.innerHTML = desktopSlider.outerHTML;
          }

          // Render mobile slider
          const mobileSlider = slideContainer.querySelector(".s-side-cart__slider--mobile");
          if (this.sliderRenderContainerMobile && mobileSlider) {
            this.sliderRenderContainerMobile.innerHTML = mobileSlider.outerHTML;
          }

          // Update slider buttons visibility
          const sliderButtonsDesktop = this.sliderWrapperDesktop?.querySelector(".js-side-cart__slider-buttons");
          const sliderButtonsMobile = this.sliderWrapperMobile?.querySelector(".js-side-cart__slider-buttons");

          if (slideCounts < 3) {
            sliderButtonsDesktop?.classList.add("is-hidden-mobile");
            sliderButtonsMobile?.classList.add("is-hidden-mobile");
          } else {
            sliderButtonsDesktop?.classList.remove("is-hidden-mobile");
            sliderButtonsMobile?.classList.remove("is-hidden-mobile");
            this.sliderWrapperDesktop?.classList.remove("is-hidden");
            this.sliderWrapperMobile?.classList.remove("is-hidden");
          }

          this.sliderWrapperDesktop?.classList.remove("is-loading");
          this.sliderWrapperMobile?.classList.remove("is-loading");
        } catch (e) {
      ...
      renderYMAL = (templateName, isLoading = false) => {
        ...
        if (!this.sliderRenderContainerDesktop && !this.sliderRenderContainerMobile) {
          this.sliderWrapperDesktop?.classList.add("is-hidden");
          this.sliderWrapperMobile?.classList.add("is-hidden");
          return;
        }
        ...
        if (this.sliderRenderContainerDesktop) {
          this.sliderRenderContainerDesktop.innerHTML = html;
        }
        if (this.sliderRenderContainerMobile) {
          this.sliderRenderContainerMobile.innerHTML = html;
        }

        this.sliderWrapperDesktop?.classList.toggle("is-loading", isLoading);
        this.sliderWrapperMobile?.classList.toggle("is-loading", isLoading);
        this.sliderWrapperDesktop?.classList.remove("is-hidden");
        this.sliderWrapperMobile?.classList.remove("is-hidden");
      };
      ...
      compileTemplate = (templateName, params = {}) => {
        switch (templateName) {
          case "cart-summary": {
            const mainTpl = document.getElementById("t-side-cart-summary-template")?.innerHTML || "";
            const discountTpl = document.getElementById("t-side-cart-discount-template")?.innerHTML || "";
          ...
          case "delivery-progress": {
            const tpl = document.getElementById("t-side-cart-delivery-progress-template")?.innerHTML || "";
            return tpl.replace("{money}", params.money || "");
          }

          case "delivery-complete": {
            const tpl = document.getElementById("t-side-cart-delivery-complete-template")?.innerHTML || "";
            return tpl;
          }

          case "ymal-skeleton": {
            const template = this.querySelector(".t-side-cart__ymal-skeleton");
            return template ? template.innerHTML : "";
          }

          case "ymal-fallback": {
            const template = this.querySelector(".t-side-cart__ymal-fallback");
            return template ? template.innerHTML : "";
          }
        }
      };
      ...
      compareCartWithDOM = (cart) => {
        const domKeys = Array.from(this.querySelectorAll(".js-side-cart__item"))
      ...
      calculateSaleDiscount = (cart) => {
        ...
        cart.items.forEach((cartItem) => {
          ...
          const domItem = this.querySelector(`.js-side-cart__item[data-key="${key}"]`);
          if (!domItem) return;
